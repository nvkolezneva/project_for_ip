...
export class DataBaseService extends BaseApi {
  ...
  async getProduct(): Promise<Product[]> {
    const jsonArray = await this.get('products', this.options).toPromise();
    const objects: Product[] = [];
    for (const json of jsonArray) {
      const object = new Product(json);
      objects.push(object);
    }
    return objects;
  }
  ...
  async createProducts(product: Product) {
    return this.post('products', product.toJSON(), this.options).toPromise();
  }
